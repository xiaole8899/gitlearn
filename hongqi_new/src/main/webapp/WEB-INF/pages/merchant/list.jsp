...
<div class="pageHeader">
	 <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="${CTX }/merchant/getMerchantList.do" method="post">
		 <div class="searchBar">
...
<div class="pageContent">
	...
	<table class="table" width="100%" layoutH="135">
		<thead>
			<tr>
				<th>序号</th>
				<th orderfield="comName" class="<c:if test="${order.field eq 'comName'}">${order.direction }</c:if>">字号名称</th>
				<th orderfield="comName" class="<c:if test="${order.field eq 'comName'}">${order.direction }</c:if>">企业名称</th>
				<th orderfield="bizName" class="<c:if test="${order.field eq 'bizName'}">${order.direction }</c:if>">持卡人</th>
		...
		<c:forEach var="merchant" items="${page.rows }" varStatus="seq">
			<tr target="m_id" rel="${merchant.bizId }">
				...
				<td>
					<div style="padding-left: 20px;">
						<c:if test="${ empty merchant.uploadTime }">
							<a class="btnEdit" href="${CTX }/merchant/toUpdateMerchant.do?mId=${merchant.bizId }" target="dialog" width="520" height="460" title="编辑商户信息">编辑</a>
							<a class="btnDel" href="${CTX }/merchant/deleteMerchant.do?mId=${merchant.bizId }" target="ajaxTodo" title="确定要注销此用户吗?">删除</a>
							<a class="btnView"  href="${CTX }/merchant/toDetailsMerchant.do?mId=${merchant.bizId }" target="dialog" width="450" height="500" rel="details" title="查看详情" maxable="false">详情</a>
						</c:if>
						<c:if test="${ not empty merchant.uploadTime }">
							<a class="btnView"  href="${CTX }/merchant/toDetailsMerchant.do?mId=${merchant.bizId }" target="dialog" width="450" height="500" title="查看详情" maxable="false">详情</a>
						</c:if>
					</div>
				</td>
			</tr>
	...
	<%-- 分页查询的Form --%>
	<form id="pagerForm" action="${CTX }/merchant/getMerchantList.do" method="post">
