...
<div class="pageHeader">
	 <form rel="pagerForm" onsubmit="return dialogSearch(this);" action="${CTX }/approach/findMerchant.do?approachBizType=${approachState}" method="post">
		 <div class="searchBar">
...
<div class="pageContent">
	...
	<%-- 数据列表 --%>
	<table class="table" targetType="dialog" width="100%" layoutH="110">
		<thead>
			<tr>
				<th>序号</th>
		...
		<c:forEach var="merchant" items="${page.rows }" varStatus="seq">
			<tr target="m_id" rel="${merchant.bizId }" style="cursor: pointer;" ondblclick="$.bringBack({businessType:'${merchant.businessType }', bizNo:'${merchant.bizNo }', bizName:'${merchant.bizName }'})">
				<td align="center">${seq.count +(page.pageIndex-1) * page.pageSize }</td>
				<td align="center">
				<a  href="${CTX }/merchant/toDetailsMerchant.do?mId=${merchant.bizId }" target="dialog" width="440" height="500" title="查看详情" maxable="false">
					${merchant.bizName }
				</a>
				</td>
				...
				<td>
			       <a class="btnSelect" href="javascript:$.bringBack({businessType:'${merchant.businessType }', bizNo:'${merchant.bizNo }', bizName:'${merchant.bizName }'})" title="查找带回">选择</a>
				</td>
			</tr>
		</c:forEach>
	...
	<%-- 分页查询的Form --%>
	<form id="pagerForm" action="${CTX }/approach/findMerchant.do?approachBizType=${approachState}"
	 method="post">
